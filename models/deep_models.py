...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.y_order1 = tf.add(tf.reduce_sum(tf.multiply(self.w_order1, X_value), 1), self.weights['b'])  # None * 1
            ...
            # FM的二阶项和
            self.y_order2 = 0.5 * tf.reduce_sum(tf.subtract(self.sum_vx_square, self.square_sum_vx), axis=1,
                                                keepdims=True)  # None * 1

            # ---------- Deep部分 ----------
            dropout_keep_deep = tf.cond(self.train_phase, lambda: self.dropout_deep,
                                        lambda: [1.0] * len(self.dropout_deep))
            self.y_deep = tf.reshape(self.embeddings, shape=[-1, self.field_size * self.embedding_size])  # None * (F*K)
            self.y_deep = tf.nn.dropout(self.y_deep, dropout_keep_deep[0])
            for i in range(0, len(self.deep_layers)):
                self.y_deep = tf.add(tf.matmul(self.y_deep, self.weights[f"W_layer_{i}"]),
                                     self.weights[f"b_layer_{i}"])  # None * layer[i] * 1
                if self.batch_norm:
                    self.y_deep = self.batch_norm_layer(self.y_deep, train_phase=self.train_phase,
                                                        scope_bn=f"bn_{i}")  # None * layer[i] * 1
    ...
    def _initialize_model_weights(self):
        ...
        for i in range(layer_cnt):
            ...
            weights[f"b_layer_{i}"] = tf.Variable(tf.zeros((1, self.deep_layers[i])), dtype=np.float32,
                                                  name=f"bias_layer_{i}")
        # DNN最后一层
